...
export default function ReportsPage() {
  ...
  const [payrollRevenueData, setPayrollRevenueData] = useState<any[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [plRes, salesRes, prRes] = await Promise.all([
        api.get('/reports/profit-loss'),
        api.get('/reports/sales'),
        api.get('/reports/payroll-vs-revenue')
      ]);
      setProfitLoss(plRes.data);
      setSalesData(salesRes.data);
      setPayrollRevenueData(prRes.data);
    } catch (err) {
      ...
      setPayrollRevenueData([
        { month: 'Jan 2026', revenue: 45000, payroll: 12000, ratio: 26.6 },
        { month: 'Feb 2026', revenue: 48000, payroll: 12000, ratio: 25.0 },
        { month: 'Mar 2026', revenue: 51000, payroll: 13500, ratio: 26.4 },
        { month: 'Apr 2026', revenue: 49000, payroll: 13500, ratio: 27.5 },
        { month: 'May 2026', revenue: 53000, payroll: 14000, ratio: 26.4 },
        { month: 'Jun 2026', revenue: 12000, payroll: 14000, ratio: 116.6 },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-full gap-8 animate-in fade-in duration-500 pb-12">
      ...
      {/* Payroll vs Revenue */}
      <div className="glass-card p-6 rounded-2xl border border-border mt-6">
        <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
          <span>👥</span> Monthly Payroll vs. Revenue (Last 6 Months)
        </h2>
        
        {loading ? (
          <div className="h-64 flex items-center justify-center text-text-muted">Loading chart data...</div>
        ) : payrollRevenueData.length === 0 ? (
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-surface/50 border-b border-border">
                <tr>
                  ...
                  <th className="p-4 font-semibold text-text-muted text-right">Payroll</th>
                  <th className="p-4 font-semibold text-text-muted text-right">Payroll/Rev Ratio</th>
                </tr>
              </thead>
              <tbody>
                {payrollRevenueData.map((d, i) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-white/5 transition-colors">
                    <td className="p-4 font-medium text-white">{d.month}</td>
                    <td className="p-4 text-right font-bold text-emerald-400">{formatMoney(d.revenue)}</td>
                    <td className="p-4 text-right font-bold text-amber-400">{formatMoney(d.payroll)}</td>
                    <td className="p-4 text-right">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold border ${d.ratio > 40 ? 'bg-rose-500/10 text-rose-400 border-rose-500/30' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30'}`}>
                        {d.ratio}%
                      </span>
                    </td>
                  </tr>

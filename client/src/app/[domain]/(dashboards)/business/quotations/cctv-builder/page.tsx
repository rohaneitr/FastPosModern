...
const CATEGORY_SLOTS = [
  { id: 'dvr', label: 'DVR/NVR/XVR', icon: '📼', keyword: 'DVR' },
  { id: 'camera', label: 'Cameras (Dome/Bullet)', icon: '📹', keyword: 'Camera' },
  { id: 'storage', label: 'Storage (HDD)', icon: '💾', keyword: 'HDD' },
  { id: 'cables', label: 'Cables (Cat6/Coaxial)', icon: '🔌', keyword: 'Cable' },
  { id: 'power', label: 'Power Supply/PoE', icon: '⚡', keyword: 'Power' },
  { id: 'connectors', label: 'Connectors', icon: '🔗', keyword: 'Connector' },
  { id: 'monitor', label: 'Monitor', icon: '📺', keyword: 'Monitor' },
  { id: 'service', label: 'Installation Charge', icon: '👷', keyword: 'Service' },
];
...
export default function CctvBuilderPage() {
  ...
  const [calcCameras, setCalcCameras] = useState(4);
  const [calcRes, setCalcRes] = useState(15); // Default to 2MP/1080p -> ~15GB/day
  ...
  useEffect(() => {
    ...
    if (userJson) {
      ...
      if (user.business?.active_modules && !user.business.active_modules.includes('cctv_builder')) {
  ...
  const handleSelectProduct = (product: any) => {
    setBuildItems(prev => {
      const copy = { ...prev };
      // Allow multiple cameras, cables, connectors
      if (copy[activeSlot.id]) {
        copy[activeSlot.id].quantity += 1;
      } else {
        copy[activeSlot.id] = { ...product, quantity: activeSlot.id === 'camera' || activeSlot.id === 'cables' || activeSlot.id === 'connectors' ? 4 : 1, slot_id: activeSlot.id };
      }
      return copy;
    });
    setActiveSlot(null);
  ...
  const updateQuantity = (slotId: string, q: number) => {
    if (q <= 0) { removeSlot(slotId); return; }
    setBuildItems(prev => ({
      ...prev,
      [slotId]: { ...prev[slotId], quantity: q }
    }));
  }
  ...
  const requiredStorageTB = ((calcCameras * calcDays * calcRes) / 1024).toFixed(2);
  ...
  return (
    <div className="flex h-full gap-6 animate-in fade-in pb-10">
      ...
      <div className="flex-[3] flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-4">
        <div>
          <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-teal-400 to-emerald-400 uppercase tracking-widest">
            CCTV Security Builder
          </h1>
          <p className="text-text-muted mt-1">Configure a surveillance system and generate a professional quotation.</p>
        </div>

        {/* Smart Storage Calculator */}
        <div className="bg-surface/50 border border-border rounded-3xl p-6 shadow-xl relative overflow-hidden mt-4">
          <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 blur-[40px] rounded-full pointer-events-none"></div>
          <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">📟 Smart Storage Calculator</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Number of Cameras</label>
              <input type="number" min="1" value={calcCameras} onChange={e => setCalcCameras(parseInt(e.target.value) || 0)} className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-white outline-none focus:border-emerald-500/50" />
            </div>
            <div>
              <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Resolution (H.265)</label>
              <select value={calcRes} onChange={e => setCalcRes(parseFloat(e.target.value))} className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-white outline-none focus:border-emerald-500/50">
                <option value={10}>1MP / 720p (~10GB/day)</option>
                <option value={15}>2MP / 1080p (~15GB/day)</option>
                <option value={20}>3MP / 2K (~20GB/day)</option>
                <option value={25}>4MP / 2K+ (~25GB/day)</option>
                <option value={40}>8MP / 4K (~40GB/day)</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Required Days</label>
              <input type="number" min="1" value={calcDays} onChange={e => setCalcDays(parseInt(e.target.value) || 0)} className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-white outline-none focus:border-emerald-500/50" />
            </div>
          </div>
          <div className="mt-6 pt-4 border-t border-border flex items-center justify-between">
            <span className="text-text-muted font-medium">Estimated HDD Required:</span>
            <span className="text-2xl font-black text-emerald-400 bg-emerald-500/10 px-4 py-2 rounded-xl border border-emerald-500/20">{requiredStorageTB} TB</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          {CATEGORY_SLOTS.map(slot => {
            ...
            return (
              <div key={slot.id} className={`p-4 rounded-2xl border transition-all ${selected ? 'bg-surface/80 border-emerald-500/50' : 'bg-surface/30 border-border hover:border-emerald-500/30'} flex flex-col gap-3 relative overflow-hidden`}>
                {selected && <div className="absolute top-0 right-0 w-20 h-20 bg-emerald-500/10 blur-[30px] rounded-full pointer-events-none"></div>}
                ...
                <div className="z-10">
                  {selected ? (
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex items-center gap-2 bg-background rounded-lg p-1 border border-border">
                          <button onClick={() => updateQuantity(slot.id, selected.quantity - 1)} className="w-6 h-6 rounded flex items-center justify-center bg-surface hover:bg-surface/80 text-white">-</button>
                    ...
                    <button 
                      ...
                      className="w-full py-4 rounded-xl border-2 border-dashed border-border hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all text-text-muted hover:text-white font-semibold flex items-center justify-center gap-2"
                    >
      ...
      <div className="flex-[1.5] bg-surface/50 border border-border rounded-3xl flex flex-col shadow-2xl relative overflow-hidden">
        ...
        <div className="p-6 border-b border-border">
          <h2 className="text-xl font-bold text-white mb-4">Quotation Summary</h2>
          
          <div className="mb-2">
            <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Customer / Client</label>
            <select value={contactId} onChange={e => setContactId(e.target.value)} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-white outline-none focus:border-emerald-500/50 transition-colors cursor-pointer">
        ...
        <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-3 custom-scrollbar">
          {buildArray.length === 0 ? (
            <div className="text-center text-text-muted mt-10">
              ...
              <p className="text-sm mt-1">Start by adding a DVR/NVR.</p>
            </div>
          ) : (
            buildArray.map((item: any) => (
              <div key={item.id} className="flex justify-between items-center">
                ...
                <div className="font-mono text-white font-bold">{format(item.price * item.quantity)}</div>
              </div>
            ))
          )}
        </div>
        ...
        <div className="p-6 bg-background/50 border-t border-border">
          <div className="space-y-2 mb-6">
            ...
            <div className="flex justify-between font-black text-2xl pt-3 border-t border-border/50 text-white mt-2">
              <span>Total</span><span className="text-emerald-400 font-mono">{format(total)}</span>
            </div>
          </div>
          <button 
            onClick={handleSaveQuotation}
            disabled={buildArray.length === 0 || isSaving}
            className="w-full bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-400 hover:to-emerald-400 disabled:opacity-50 text-white font-bold py-4 rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2 uppercase tracking-wider text-sm"
          >
          ...
          <div className="bg-surface border border-border rounded-3xl w-full max-w-4xl h-[80vh] shadow-2xl flex flex-col animate-in zoom-in-95 overflow-hidden">
            ...
            <div className="p-6 border-b border-border">
              <input 
                ...
                autoFocus
                className="w-full bg-background border border-border rounded-xl px-4 py-3 text-white outline-none focus:border-emerald-500/50 transition-colors text-lg"
              />
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 custom-scrollbar">
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map(p => (
                  <button key={p.id} onClick={() => handleSelectProduct(p)} className="bg-background border border-border hover:border-emerald-500/50 rounded-2xl p-5 text-left transition-all group flex flex-col h-40">
                    <div className="font-bold text-white leading-snug line-clamp-2 group-hover:text-emerald-400 transition-colors">{p.name}</div>
                    ...
                    <div className="mt-auto flex justify-between items-center">
                      ...
                      <div className="w-8 h-8 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">↓</div>
                    </div>
                  </button>
                ))
              )}
        ...
        <div className="hidden print:block absolute top-0 left-0 bg-white text-black z-[9999] w-[210mm] min-h-[297mm] p-[20mm] font-sans box-border">
          <div className="flex justify-between items-start border-b-2 border-black pb-8 mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-2 font-medium">CCTV System Quotation</p>
          ...
          <table className="w-full text-left border-collapse mb-8">
            ...
            <tbody>
              {receiptData.items.map((item: any, idx: number) => {
                const category = CATEGORY_SLOTS.find(s => s.id === item.slot_id)?.label || 'Component';

...
export default function SaleDetailPage() {
  ...
  const [refundMethod, setRefundMethod] = useState<'cash' | 'advance'>('cash');
  ...
  const fetchSale = async () => {
    try {
      ...
      // Initialize return lines based on existing lines
  ...
  const handleReturnQtyChange = (index: number, val: number) => {
    ...
    // reset serials if returning less than selected
  ...
  const handleProcessReturn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Return processed successfully! Inventory & serials reverted.');
      setIsReturnModalOpen(false);
      fetchSale(); // Reload sale to show updated statuses if any (though currently it creates a new sell_return transaction)
    } catch (err: any) {
  ...
  if (loading) return <div className="p-8 flex justify-center"><div className="w-8 h-8 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div></div>;
  if (error || !sale) return <div className="p-8 text-rose-500">{error || 'Sale not found'}</div>;
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex justify-between items-start">
        <div>
          <button onClick={() => router.back()} className="text-sm text-text-muted hover:text-white mb-2 block transition-colors">&larr; Back to Sales</button>
          <h1 className="text-3xl font-black text-white flex items-center gap-3">
            Invoice {sale.invoice_no}
            {sale.status === 'final' && <span className="bg-emerald-500/20 text-emerald-400 text-sm px-3 py-1 rounded-full font-bold">FINAL</span>}
          </h1>
          <p className="text-text-muted mt-1">{new Date(sale.transaction_date).toLocaleString()} • {sale.customer?.name || 'Walk-in Customer'}</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={() => setIsReturnModalOpen(true)} className="bg-rose-500 hover:bg-rose-600 text-white px-5 py-2 rounded-xl text-sm font-bold shadow-lg shadow-rose-500/20 transition-all flex items-center gap-2">
            ↩ Process Return
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card p-5 rounded-2xl border border-white/5">
          ...
          <span className="text-3xl font-black text-white">${Number(sale.final_total).toFixed(2)}</span>
        </div>
        <div className="glass-card p-5 rounded-2xl border border-white/5">
          <span className="text-text-muted text-xs uppercase font-bold tracking-wider block mb-1">Payment Status</span>
          <span className={`text-2xl font-black ${sale.payment_status === 'paid' ? 'text-emerald-400' : 'text-amber-400'} uppercase`}>{sale.payment_status}</span>
        </div>
        <div className="glass-card p-5 rounded-2xl border border-white/5">
          <span className="text-text-muted text-xs uppercase font-bold tracking-wider block mb-1">Due Amount</span>
          <span className="text-3xl font-black text-rose-400">${Number(sale.amount_due).toFixed(2)}</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="glass-card w-full max-w-2xl rounded-2xl p-0 shadow-2xl border border-white/10 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-border flex justify-between items-center bg-white/5">
              <h2 className="text-xl font-bold flex items-center gap-2">↩ Process Omnichannel Return</h2>
            ...
            <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
                    ...
                    <div key={idx} className="bg-background/50 border border-border p-4 rounded-xl">
                      ...
                      {line.return_qty > 0 && line.purchased_serials.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-border/50">
                          <span className="text-xs text-amber-400 font-semibold mb-2 block">Select {line.return_qty} serial(s) to revert:</span>
              ...
              <div className="bg-primary/10 border border-primary/20 p-4 rounded-xl">
                <h3 className="text-sm font-semibold text-primary mb-3">Refund & Ledger Management</h3>
                <div className="flex flex-col gap-3">
                  ...
                  <div>
                    <label className="text-xs font-semibold text-text-muted uppercase mb-1 block">Refund Method</label>
                    <select 
                      value={refundMethod} 
                    ...
                    >
                      <option value="cash">Cash (Deduct from Register & Return to Customer)</option>
                      <option value="advance">Wallet Advance (Add to Customer's Ledger Balance)</option>
                    </select>
                    <p className="text-xs text-text-muted mt-2">
                      {refundMethod === 'cash' 
                        ? 'This will subtract money from your active drawer/Z-Report and balance the customer ledger to 0.' 
                        : 'No cash leaves the drawer. This puts the customer in a negative-due state, allowing them to use it for future purchases.'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6 border-t border-border bg-white/5 flex justify-end gap-3 rounded-b-2xl">
              ...
              <button 
                ...
                className="px-6 py-2.5 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white rounded-lg font-bold shadow-lg shadow-rose-600/20 transition-all flex items-center gap-2"
              >
